...
const FnConvertFlatDataToHierarchyData = async (flatJSON: any, currentNodeId: string | null = null, disableSort?: boolean) => {
    try {
        ...
        if (typeof flatJSON === "object") {
            for (let index = 0; index < Object.keys(flatJSON).length; index++) {
                ...
                if (element && element.length > 0) {
                    const convertedData = FnBuildRcTreeData(element, currentNodeId, disableSort);
